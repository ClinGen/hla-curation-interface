...
class DatatableViewTest(TestCase):
    ...
    def test_clear_pokedex_number_button(self):
        response = self.client.get(self.url, {"pokedex_number": "5"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_pokedex_number_button = (
            soup.find(id="clear-pokedex-number-button").get_text().strip()
        )
        self.assertEqual(clear_pokedex_number_button, "✗ Number: 5")

    def test_clear_name_button(self):
        response = self.client.get(self.url, {"name": "Po"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_name_button = soup.find(id="clear-name-button").get_text().strip()
        self.assertEqual(clear_name_button, "✗ Name: Po")

    def test_clear_type1_button(self):
        response = self.client.get(self.url, {"type1": "Flying"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_type1_button = soup.find(id="clear-type1-button").get_text().strip()
        self.assertEqual(clear_type1_button, "✗ Type 1: Flying")

    def test_clear_type2_button(self):
        response = self.client.get(self.url, {"type2": "Rock"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_type2_button = soup.find(id="clear-type2-button").get_text().strip()
        self.assertEqual(clear_type2_button, "✗ Type 2: Rock")

    def test_clear_sort_total_button(self):
        response = self.client.get(self.url, {"total": "asc"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_sort_total_button = soup.find(id="clear-total-button").get_text().strip()
        self.assertEqual(clear_sort_total_button, "✗ Total: asc")

    def test_clear_sort_hp_button(self):
        response = self.client.get(self.url, {"hp": "asc"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_sort_hp_button = soup.find(id="clear-hp-button").get_text().strip()
        self.assertEqual(clear_sort_hp_button, "✗ HP: asc")

    def test_clear_sort_attack_button(self):
        response = self.client.get(self.url, {"attack": "asc"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_sort_attack_button = (
            soup.find(id="clear-attack-button").get_text().strip()
        )
        self.assertEqual(clear_sort_attack_button, "✗ Attack: asc")

    def test_clear_sort_defense_button(self):
        response = self.client.get(self.url, {"defense": "asc"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_sort_defense_button = (
            soup.find(id="clear-defense-button").get_text().strip()
        )
        self.assertEqual(clear_sort_defense_button, "✗ Defense: asc")

    def test_clear_sort_sp_atk_button(self):
        response = self.client.get(self.url, {"sp_atk": "asc"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_sort_sp_atk_button = (
            soup.find(id="clear-sp-atk-button").get_text().strip()
        )
        self.assertEqual(clear_sort_sp_atk_button, "✗ Sp. Atk: asc")

    def test_clear_sort_sp_def_button(self):
        response = self.client.get(self.url, {"sp_def": "asc"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_sort_sp_def_button = (
            soup.find(id="clear-sp-def-button").get_text().strip()
        )
        self.assertEqual(clear_sort_sp_def_button, "✗ Sp. Def: asc")

    def test_clear_sort_speed_button(self):
        response = self.client.get(self.url, {"speed": "asc"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_sort_speed_button = soup.find(id="clear-speed-button").get_text().strip()
        self.assertEqual(clear_sort_speed_button, "✗ Speed: asc")

    def test_clear_generation_button(self):
        response = self.client.get(self.url, {"generation": "6"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_generation_button = (
            soup.find(id="clear-generation-button").get_text().strip()
        )
        self.assertEqual(clear_generation_button, "✗ Generation: 6")

    def test_clear_legendary_button(self):
        response = self.client.get(self.url, {"legendary": "False"})
        soup = BeautifulSoup(response.content, "html.parser")
        clear_legendary_button = (
            soup.find(id="clear-legendary-button").get_text().strip()
        )
        self.assertEqual(clear_legendary_button, "✗ Legendary: False")
